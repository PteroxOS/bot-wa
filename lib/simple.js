  ...
  for (const reaction of reactions) {
    try {
      const m = {
        ...
        isGroup: reaction.key.remoteJid.endsWith('@g.us'),
        isBotAdmin: false // diisi di handler nanti
      }

      // inject ke handler BEFORE
...
Object.defineProperty(conn, 'sendGroupStatus', {
...
`.trim()
    ...
    */
    ...
    // ===============================
// NATIVE FLOW INTERACTIVE BUTTON
// (SAFE / WESKER STYLE)
// ===============================
const { proto } = require('@adiwajshing/baileys')

conn.sendButtonNativeFlow = async (jid, content = {}, options = {}) => {
  ...
  // ===============================
  // RELAY (BYPASS sendMessage)
  // ===============================
  return await conn.relayMessage(jid, message, {
    quoted: options.quoted,
    additionalNodes
  })
}

conn.sendButtonV2Full = async (jid, content = {}, options = {}) => {
  if (!conn.user?.id) throw new Error('User not authenticated')
...
conn.sendButtonV2 = async (jid, content = {}, options = {}) => {
  const msg = {
    viewOnceMessage: {
      message: {
